...
describe MagicSeven do
  ...
  describe '#subtract_four' do
    context 'when the previous step is 10' do
      it 'returns 6' do 
        previous_step = 10 # A
        result = game.subtract_four(previous_step) # A
  ...
  describe '#subtract_random_number' do
    ...
    context 'when the random number is 10 and specific number is 16' do 
      it 'returns 6' do 
        random_number = game.random_number
        specific_number = 16
        result = game.subtract_random_number(specific_number)
        expect(result).to eq(6)
      end
    end
  end
  ...
  describe '#play' do
    context 'when the random number is 17' do
      subject(:game) { described_class.new(17) }
    ...
    context 'when the random number is 32' do
      # remove the 'x' before running this test
      subject(:game) { described_class.new(32) }
      it 'will return 7' do
        result = game.play
        expect(result).to eq(7)
      end
    ...
    context 'when the random number is 4231' do
      # remove the 'x' before running this test
      subject(:game) { described_class.new(4231) }
      it 'will return 7' do
        result = game.play
        expect(result).to eq(7)
      end
