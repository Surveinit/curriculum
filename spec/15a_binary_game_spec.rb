...
# 4. Looping Script Method -> Test the behavior of the method, if its stops correctly.
# -----------------------------------------------------------------
...
describe BinaryGame do
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        verify_input = game_verify.verify_input(min, max, valid_input)
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        verify_input = game_verify.verify_input(min, max, invalid_input)
  ...
  describe '#display_turn_order' do
    ...
    subject(:game_display_turn) {described_class.new(1, 10)}
    ...
    before do
      ...
      allow(game_display_turn).to receive(:display_guess)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect { game_display_turn.display_turn_order(binary_search_turn) }.to change { game_display_turn.instance_variable_get(:@guess_count) }.by(1) 
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary_search_turn).to receive(:make_guess).once
      game_display_turn.display_turn_order(binary_search_turn)
    end
